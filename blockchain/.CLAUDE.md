...
Before writing or modifying test files (\*.test.ts), you MUST invoke:

- /solidity-testing

Before deploying contracts or writing Ignition modules (ignition/modules/\*.ts) or scripts (scripts/\*.ts), you MUST invoke:

- /solidity-deploy
- /defi-security

Before any on-chain operation (pnpm deploy:*, npx hardhat run scripts/), you MUST invoke:
...
- Solidity projects: `pnpm build` + `pnpm test` must pass
